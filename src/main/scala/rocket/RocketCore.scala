...
case class RocketCoreParams(bootFreqHz: BigInt = 0, useVM: Boolean = true, useUser: Boolean = false, useSupervisor: Boolean = false, useDebug: Boolean = true, useAtomics: Boolean = true, useAtomicsOnlyForIO: Boolean = false, useCompressed: Boolean = true, useRVE: Boolean = false, useSCIE: Boolean = false, nLocalInterrupts: Int = 0, useNMI: Boolean = false, nBreakpoints: Int = 1, useBPWatch: Boolean = false, mcontextWidth: Int = 0, scontextWidth: Int = 0, nPMPs: Int = 8, haveCFlush: Boolean = false, misaWritable: Boolean = true, nL2TLBEntries: Int = 0, nL2TLBWays: Int = 1, mtvecInit: Option[BigInt] = Some(BigInt(0)), mtvecWritable: Boolean = true, fastLoadWord: Boolean = true, fastLoadByte: Boolean = false, branchPredictionModeCSR: Boolean = false, clockGate: Boolean = false, mvendorid: Int = 0, mimpid: Int = 538447876, mulDiv: Option[MulDivParams] = Some(MulDivParams()), fpu: Option[FPUParams] = Some(FPUParams())) extends CoreParams {
  val lgPauseCycles = 5
...
class RocketCustomCSRs(implicit p: Parameters) extends CustomCSRs with HasRocketCoreParameters {
  ...
  override def chickenCSR = {
    val mask = BigInt(tileParams.dcache.get.clockGate.toInt << 0 | rocketParams.clockGate.toInt << 1 | rocketParams.clockGate.toInt << 2 | 1 << 3 | haveDCache.toInt << 9 | tileParams.icache.get.prefetch.toInt << 17)
...
@chiselName class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p) with HasRocketCoreParameters with HasCoreIO {
  ...
  val gated_clock = if (!rocketParams.clockGate) clock else ClockGate(clock, clock_en, "rocket_clock_gate")
  @chiselName class RocketImpl extends NoChiselNamePrefix {
    def pipelineIDToWB[T <: Data](x: T): T = RegEnable(RegEnable(RegEnable(x, !ctrl_killd), ex_pc_valid), mem_pc_valid)
    val pipelinedMul = usingMulDiv && mulDivParams.mulUnroll == xLen
    val decode_table = {
      require(!usingRoCC || !rocketParams.useSCIE)
      (if (usingMulDiv) new MDecode(pipelinedMul) +: (xLen > 32).option(new M64Decode(pipelinedMul)).toSeq else Nil) ++: (if (usingAtomics) new ADecode +: (xLen > 32).option(new A64Decode).toSeq else Nil) ++: (if (fLen >= 32) new FDecode +: (xLen > 32).option(new F64Decode).toSeq else Nil) ++: (if (fLen >= 64) new DDecode +: (xLen > 32).option(new D64Decode).toSeq else Nil) ++: (if (minFLen == 16) new HDecode +: (xLen > 32).option(new H64Decode).toSeq ++: (fLen >= 64).option(new HDDecode).toSeq else Nil) ++: usingRoCC.option(new RoCCDecode) ++: rocketParams.useSCIE.option(new SCIEDecode) ++: (if (xLen == 32) new I32Decode else new I64Decode) +: usingVM.option(new SVMDecode) ++: usingSupervisor.option(new SDecode) ++: usingDebug.option(new DebugDecode) ++: usingNMI.option(new NMIDecode) ++: Seq(new FenceIDecode(tile.dcache.flushOnFenceI)) ++: coreParams.haveCFlush.option(new CFlushDecode(tile.dcache.canSupportCFlushLine)) ++: Seq(new IDecode)
    } flatMap (_.table)
    val perfEvents = new EventSets()
    val instEvents = new EventSet((mask, hits) => Mux(wb_xcpt, mask(0), wb_valid && pipelineIDToWB((mask & hits).orR)), 18)
    instEvents.addEvent("exception", () => false.B, 0)
    instEvents.addEvent("load", () => id_ctrl.mem && id_ctrl.mem_cmd === M_XRD && !id_ctrl.fp, 1)
    instEvents.addEvent("store", () => id_ctrl.mem && id_ctrl.mem_cmd === M_XWR && !id_ctrl.fp, 2)
    instEvents.addEvent("amo", () => Bool(usingAtomics) && id_ctrl.mem && (isAMO(id_ctrl.mem_cmd) || id_ctrl.mem_cmd.isOneOf(M_XLR, M_XSC)), 3)
    instEvents.addEvent("system", () => id_ctrl.csr =/= CSR.N, 4)
    instEvents.addEvent("arith", () => id_ctrl.wxd && !(id_ctrl.jal || id_ctrl.jalr || id_ctrl.mem || id_ctrl.fp || id_ctrl.mul || id_ctrl.div || id_ctrl.csr =/= CSR.N), 5)
    instEvents.addEvent("branch", () => id_ctrl.branch, 6)
    instEvents.addEvent("jal", () => id_ctrl.jal, 7)
    instEvents.addEvent("jalr", () => id_ctrl.jalr, 8)
    if (usingMulDiv) {
      instEvents.addEvent("mul", () => if (pipelinedMul) id_ctrl.mul else id_ctrl.div && (id_ctrl.alu_fn & ALU.FN_DIV) =/= ALU.FN_DIV, 9)
      instEvents.addEvent("div", () => if (pipelinedMul) id_ctrl.div else id_ctrl.div && (id_ctrl.alu_fn & ALU.FN_DIV) === ALU.FN_DIV, 10)
    }
    if (usingFPU) {
      ...
      instEvents.addEvent("fp mul", () => id_ctrl.fp && io.fpu.dec.fma && !io.fpu.dec.swap23 && !io.fpu.dec.ren3, 14)
      ...
      instEvents.addEvent("fp other", () => id_ctrl.fp && !(io.fpu.dec.ldst || io.fpu.dec.fma || io.fpu.dec.div || io.fpu.dec.sqrt), 17)
    }
    perfEvents.addEventSet(instEvents)
    val microEvents = new EventSet((mask, hits) => (mask & hits).orR, 11)
    microEvents.addEvent("load-use interlock", () => id_ex_hazard && ex_ctrl.mem || id_mem_hazard && mem_ctrl.mem || id_wb_hazard && wb_ctrl.mem, 0)
    microEvents.addEvent("long-latency interlock", () => id_sboard_hazard, 1)
    microEvents.addEvent("csr interlock", () => id_ex_hazard && ex_ctrl.csr =/= CSR.N || id_mem_hazard && mem_ctrl.csr =/= CSR.N || id_wb_hazard && wb_ctrl.csr =/= CSR.N, 2)
    ...
    microEvents.addEvent("control-flow target misprediction", () => take_pc_mem && mem_misprediction && mem_cfi && !mem_direction_misprediction && !icache_blocked, 6)
    microEvents.addEvent("flush", () => wb_reg_flush_pipe, 7)
    microEvents.addEvent("replay", () => replay_wb, 8)
    if (usingMulDiv) {
      microEvents.addEvent("mul/div interlock", () => id_ex_hazard && (ex_ctrl.mul || ex_ctrl.div) || id_mem_hazard && (mem_ctrl.mul || mem_ctrl.div) || id_wb_hazard && wb_ctrl.div, 9)
    }
    if (usingFPU) {
      microEvents.addEvent("fp interlock", () => id_ex_hazard && ex_ctrl.fp || id_mem_hazard && mem_ctrl.fp || id_wb_hazard && wb_ctrl.fp || id_ctrl.fp && id_stall_fpu, 10)
    }
    ...
    val id_illegal_insn = !id_ctrl.legal || (id_ctrl.mul || id_ctrl.div) && !csr.io.status.isa('m' - 'a') || id_ctrl.amo && !csr.io.status.isa('a' - 'a') || id_ctrl.fp && (csr.io.decode(0).fp_illegal || io.fpu.illegal_rm) || id_ctrl.dp && !csr.io.status.isa('d' - 'a') || ibuf.io.inst(0).bits.rvc && !csr.io.status.isa('c' - 'a') || id_raddr2_illegal && !id_ctrl.scie && id_ctrl.rxs2 || id_raddr1_illegal && !id_ctrl.scie && id_ctrl.rxs1 || id_waddr_illegal && !id_ctrl.scie && id_ctrl.wxd || id_ctrl.rocc && csr.io.decode(0).rocc_illegal || id_ctrl.scie && !(id_scie_decoder.unpipelined || id_scie_decoder.pipelined) || id_csr_en && (csr.io.decode(0).read_illegal || !id_csr_ren && csr.io.decode(0).write_illegal) || !ibuf.io.inst(0).bits.rvc && ((id_sfence || id_system_insn) && csr.io.decode(0).system_illegal)
    ...
    val id_rocc_busy = Bool(usingRoCC) && (io.rocc.busy || ex_reg_valid && ex_ctrl.rocc || mem_reg_valid && mem_ctrl.rocc || wb_reg_valid && wb_ctrl.rocc)
    val id_do_fence = Wire(init = id_rocc_busy && id_ctrl.fence || id_mem_busy && (id_ctrl.amo && id_amo_rl || id_ctrl.fence_i || id_reg_fence && (id_ctrl.mem || id_ctrl.rocc)))
    val bpu = Module(new BreakpointUnit(nBreakpoints))
    val id_xcpt0 = ibuf.io.inst(0).bits.xcpt0
    val id_xcpt1 = ibuf.io.inst(0).bits.xcpt1
    val (id_xcpt, id_cause) = checkExceptions(List((csr.io.interrupt, csr.io.interrupt_cause), (bpu.io.debug_if, UInt(CSR.debugTriggerCause)), (bpu.io.xcpt_if, UInt(Causes.breakpoint)), (id_xcpt0.pf.inst, UInt(Causes.fetch_page_fault)), (id_xcpt0.ae.inst, UInt(Causes.fetch_access)), (id_xcpt1.pf.inst, UInt(Causes.fetch_page_fault)), (id_xcpt1.ae.inst, UInt(Causes.fetch_access)), (id_illegal_insn, UInt(Causes.illegal_instruction))))
    val idCoverCauses = List((CSR.debugTriggerCause, "DEBUG_TRIGGER"), (Causes.breakpoint, "BREAKPOINT"), (Causes.fetch_access, "FETCH_ACCESS"), (Causes.illegal_instruction, "ILLEGAL_INSTRUCTION")) ++ (if (usingVM) List((Causes.fetch_page_fault, "FETCH_PAGE_FAULT")) else Nil)
    val dcache_bypass_data = if (fastLoadByte) io.dmem.resp.bits.data(xLen - 1, 0) else if (fastLoadWord) io.dmem.resp.bits.data_word_bypass(xLen - 1, 0) else wb_reg_wdata
    ...
    val bypass_sources = IndexedSeq((Bool(true), UInt(0), UInt(0)), (ex_reg_valid && ex_ctrl.wxd, ex_waddr, mem_reg_wdata), (mem_reg_valid && mem_ctrl.wxd && !mem_ctrl.mem, mem_waddr, wb_reg_wdata), (mem_reg_valid && mem_ctrl.wxd, mem_waddr, dcache_bypass_data))
    ...
    val ex_rs = for (i <- 0 until id_raddr.size) yield Mux(ex_reg_rs_bypass(i), bypass_mux(ex_reg_rs_lsb(i)), Cat(ex_reg_rs_msb(i), ex_reg_rs_lsb(i)))
    val ex_imm = ImmGen(ex_ctrl.sel_imm, ex_reg_inst)
    val ex_op1 = MuxLookup(ex_ctrl.sel_alu1, SInt(0), Seq(A1_RS1 -> ex_rs(0).asSInt, A1_PC -> ex_reg_pc.asSInt))
    val ex_op2 = MuxLookup(ex_ctrl.sel_alu2, SInt(0), Seq(A2_RS2 -> ex_rs(1).asSInt, A2_IMM -> ex_imm, A2_SIZE -> Mux(ex_reg_rvc, SInt(2), SInt(4))))
    ...
    val replay_ex_structural = ex_ctrl.mem && !io.dmem.req.ready || ex_ctrl.div && !div.io.req.ready
    ...
    val (ex_xcpt, ex_cause) = checkExceptions(List((ex_reg_xcpt_interrupt || ex_reg_xcpt, ex_reg_cause)))
    val exCoverCauses = idCoverCauses
    buildExecuteStage()
    val mem_pc_valid = mem_reg_valid || mem_reg_replay || mem_reg_xcpt_interrupt
    val mem_br_target = mem_reg_pc.asSInt + Mux(mem_ctrl.branch && mem_br_taken, ImmGen(IMM_SB, mem_reg_inst), Mux(mem_ctrl.jal, ImmGen(IMM_UJ, mem_reg_inst), Mux(mem_reg_rvc, SInt(2), SInt(4))))
    ...
    val mem_wrong_npc = Mux(ex_pc_valid, mem_npc =/= ex_reg_pc, Mux(ibuf.io.inst(0).valid || ibuf.io.imem.valid, mem_npc =/= ibuf.io.pc, Bool(true)))
    ...
    val (mem_ldst_xcpt, mem_ldst_cause) = checkExceptions(List((mem_debug_breakpoint, UInt(CSR.debugTriggerCause)), (mem_breakpoint, UInt(Causes.breakpoint))))
    val (mem_xcpt, mem_cause) = checkExceptions(List((mem_reg_xcpt_interrupt || mem_reg_xcpt, mem_reg_cause), (mem_reg_valid && mem_npc_misaligned, UInt(Causes.misaligned_fetch)), (mem_reg_valid && mem_ldst_xcpt, mem_ldst_cause)))
    val memCoverCauses = (exCoverCauses ++ List((CSR.debugTriggerCause, "DEBUG_TRIGGER"), (Causes.breakpoint, "BREAKPOINT"), (Causes.misaligned_fetch, "MISALIGNED_FETCH"))).distinct
    val dcache_kill_mem = mem_reg_valid && mem_ctrl.wxd && io.dmem.replay_next
    ...
    buildMemoryStage()
    val (wb_xcpt, wb_cause) = checkExceptions(List((wb_reg_xcpt, wb_reg_cause), (wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ma.st, UInt(Causes.misaligned_store)), (wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ma.ld, UInt(Causes.misaligned_load)), (wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.pf.st, UInt(Causes.store_page_fault)), (wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.pf.ld, UInt(Causes.load_page_fault)), (wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ae.st, UInt(Causes.store_access)), (wb_reg_valid && wb_ctrl.mem && io.dmem.s2_xcpt.ae.ld, UInt(Causes.load_access))))
    val wbCoverCauses = List((Causes.misaligned_store, "MISALIGNED_STORE"), (Causes.misaligned_load, "MISALIGNED_LOAD"), (Causes.store_access, "STORE_ACCESS"), (Causes.load_access, "LOAD_ACCESS")) ++ (if (usingVM) List((Causes.store_page_fault, "STORE_PAGE_FAULT"), (Causes.load_page_fault, "LOAD_PAGE_FAULT")) else Nil)
    ...
    val rf_wdata = Mux(dmem_resp_valid && dmem_resp_xpu, io.dmem.resp.bits.data(xLen - 1, 0), Mux(ll_wen, ll_wdata, Mux(wb_ctrl.csr =/= CSR.N, csr.io.rw.rdata, Mux(wb_ctrl.mul, mul.map(_.io.resp.bits.data).getOrElse(wb_reg_wdata), wb_reg_wdata))))
    buildWritebackStage()
    val tval_valid = wb_xcpt && wb_cause.isOneOf(Causes.illegal_instruction, Causes.breakpoint, Causes.misaligned_load, Causes.misaligned_store, Causes.load_access, Causes.store_access, Causes.fetch_access, Causes.load_page_fault, Causes.store_page_fault, Causes.fetch_page_fault)
    hookUpCSR()
    val hazard_targets = Seq((id_ctrl.rxs1 && id_raddr1 =/= UInt(0), id_raddr1), (id_ctrl.rxs2 && id_raddr2 =/= UInt(0), id_raddr2), (id_ctrl.wxd && id_waddr =/= UInt(0), id_waddr))
    val fp_hazard_targets = Seq((io.fpu.dec.ren1, id_raddr1), (io.fpu.dec.ren2, id_raddr2), (io.fpu.dec.ren3, id_raddr3), (io.fpu.dec.wen, id_waddr))
    val sboard = new Scoreboard(32, true)
    sboard.clear(ll_wen, ll_waddr)
    def id_sboard_clear_bypass(r: UInt) = {
      if (!tileParams.dcache.get.dataECC.isDefined) ll_wen && ll_waddr === r else div.io.resp.fire() && div.io.resp.bits.tag === r || dmem_resp_replay && dmem_resp_xpu && dmem_resp_waddr === r
    }
    ...
    val mem_mem_cmd_bh = if (fastLoadWord) Bool(!fastLoadByte) && mem_reg_slow_bypass else Bool(true)
    ...
    val ctrl_stalld = id_ex_hazard || id_mem_hazard || id_wb_hazard || id_sboard_hazard || csr.io.singleStep && (ex_reg_valid || mem_reg_valid || wb_reg_valid) || id_csr_en && csr.io.decode(0).fp_csr && !io.fpu.fcsr_rdy || id_ctrl.fp && id_stall_fpu || id_ctrl.mem && dcache_blocked || id_ctrl.rocc && rocc_blocked || id_ctrl.div && (!(div.io.req.ready || div.io.resp.valid && !wb_wxd) || div.io.req.valid) || !clock_en || id_do_fence || csr.io.csr_stall || id_reg_pause || io.traceStall
    ctrl_killd := !ibuf.io.inst(0).valid || ibuf.io.inst(0).bits.replay || take_pc_mem_wb || ctrl_stalld || csr.io.interrupt
    val ex_dcache_tag = Cat(ex_waddr, ex_ctrl.fp)
    val icache_blocked = !(io.imem.resp.valid || RegNext(io.imem.resp.valid))
    hookUpCore()
    csr.io.counters foreach ({
      c => c.inc := RegNext(perfEvents.evaluate(c.eventSel))
    })
    ...
    if (rocketParams.clockGate) {
      ...
      clock_en_reg := ex_pc_valid || mem_pc_valid || wb_pc_valid || io.ptw.customCSRs.disableCoreClockGate || !div.io.req.ready || usingFPU && !io.fpu.fcsr_rdy || io.dmem.replay_next || !long_latency_stall && (ibuf.io.inst(0).valid || io.imem.resp.valid)
...
""", io.hartid, coreMonitorBundle.timer, coreMonitorBundle.valid, coreMonitorBundle.pc, Mux(wb_ctrl.wxd || wb_ctrl.wfd, coreMonitorBundle.wrdst, 0.U), Mux(coreMonitorBundle.wrenx, coreMonitorBundle.wrdata, 0.U), coreMonitorBundle.wrenx, Mux(wb_ctrl.rxs1 || wb_ctrl.rfs1, coreMonitorBundle.rd0src, 0.U), Mux(wb_ctrl.rxs1 || wb_ctrl.rfs1, coreMonitorBundle.rd0val, 0.U), Mux(wb_ctrl.rxs2 || wb_ctrl.rfs2, coreMonitorBundle.rd1src, 0.U), Mux(wb_ctrl.rxs2 || wb_ctrl.rfs2, coreMonitorBundle.rd1val, 0.U), coreMonitorBundle.inst, coreMonitorBundle.inst)
      }
    }
    ...
    PlusArg.timeout(name = "max_core_cycles", docstring = "Kill the emulation after INT rdtime cycles. Off if 0.")(csr.io.time)
    def buildDecodeStage(): Unit = {
    ...
    def buildExecuteStage(): Unit = {
      ...
      when(!ctrl_killd) {
        ...
        when(id_ctrl.fence && id_fence_succ === 0) {
          id_reg_pause := true
        }
        when(id_fence_next) {
          id_reg_fence := true
        ...
        when(id_xcpt) {
          ...
          when(id_xcpt1.asUInt.orR) {
            ex_ctrl.sel_alu1 := A1_PC
            ex_ctrl.sel_alu2 := A2_SIZE
            ex_reg_rvc := true
          }
          when(bpu.io.xcpt_if || id_xcpt0.asUInt.orR) {
            ex_ctrl.sel_alu1 := A1_PC
            ex_ctrl.sel_alu2 := A2_ZERO
          }
        }
      ...
      when(!ctrl_killd || csr.io.interrupt || ibuf.io.inst(0).bits.replay) {
        ...
        ex_reg_wphit := bpu.io.bpwatch.map {
          bpw => bpw.ivalid(0)
        }
      }
      coverExceptions(ex_xcpt, ex_cause, "EXECUTE", exCoverCauses)
    }
    def buildMemoryStage(): Unit = {
    ...
    def buildWritebackStage(): Unit = {
      ...
      when(mem_pc_valid) {
        ...
        wb_reg_wdata := Mux(mem_scie_pipelined, mem_scie_pipelined_wdata, Mux(!mem_reg_xcpt && mem_ctrl.fp && mem_ctrl.wxd, io.fpu.toint_data, mem_int_wdata))
        ...
        wb_reg_wphit := mem_reg_wphit | bpu.io.bpwatch.map {
          bpw => bpw.rvalid(0) && mem_reg_load || bpw.wvalid(0) && mem_reg_store
        }
      }
      ...
      when(dmem_resp_replay && dmem_resp_xpu) {
        div.io.resp.ready := Bool(false)
        if (usingRoCC) io.rocc.resp.ready := Bool(false)
    ...
    def hookUpCSR(): Unit = {
      ...
      (io.ptw.customCSRs.csrs zip csr.io.customCSRs).map({
        case (lhs, rhs) =>
          lhs := rhs
      })
    ...
    def hookUpCore(): Unit = {
      ...
      io.imem.req.bits.pc := Mux(wb_xcpt || csr.io.eret, csr.io.evec, Mux(replay_wb, wb_reg_pc, mem_npc))
      ...
      io.imem.btb_update.bits.cfiType := Mux((mem_ctrl.jal || mem_ctrl.jalr) && mem_waddr(0), CFIType.call, Mux(mem_ctrl.jalr && (mem_reg_inst(19, 15) & regAddrMask) === BitPat("b00?01"), CFIType.ret, Mux(mem_ctrl.jal || mem_ctrl.jalr, CFIType.jump, CFIType.branch)))
  ...
  def checkHazards(targets: Seq[(Bool, UInt)], cond: UInt => Bool) = targets.map(h => h._1 && cond(h._2)).reduce(_ || _)
...
class RegFile(n: Int, w: Int, zero: Boolean = false) {
  ...
  def write(addr: UInt, data: UInt) = {
    ...
    when(addr =/= UInt(0)) {
      access(addr) := data
      for ((raddr, rdata) <- reads) when(addr === raddr) {
        rdata := data
      }
    }
  }
...
object ImmGen {
  def apply(sel: UInt, inst: UInt) = {
    ...
    val b11 = Mux(sel === IMM_U || sel === IMM_Z, SInt(0), Mux(sel === IMM_UJ, inst(20).asSInt, Mux(sel === IMM_SB, inst(7).asSInt, sign)))
    val b10_5 = Mux(sel === IMM_U || sel === IMM_Z, Bits(0), inst(30, 25))
    val b4_1 = Mux(sel === IMM_U, Bits(0), Mux(sel === IMM_S || sel === IMM_SB, inst(11, 8), Mux(sel === IMM_Z, inst(19, 16), inst(24, 21))))
    val b0 = Mux(sel === IMM_S, inst(7), Mux(sel === IMM_I, inst(20), Mux(sel === IMM_Z, inst(15), Bits(0))))
