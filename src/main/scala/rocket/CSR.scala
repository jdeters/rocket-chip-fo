...
class CSRFileIO(implicit p: Parameters) extends CoreBundle with HasCoreParameters {
  ...
  val counters = Vec(0, new PerfCounterIO)
...
object VType {
  ...
  def computeVL(avl: UInt, vtype: UInt, currentVL: UInt, useCurrentVL: Bool, useMax: Bool, useZero: Bool)(implicit p: Parameters): UInt = VType.fromUInt(vtype, true).vl(avl, currentVL, useCurrentVL, useMax, useZero)
}
class VType(implicit p: Parameters) extends CoreBundle {
  ...
  @deprecated("use vlmul_sign, vlmul_mag, or vlmul_signed", "RVV 0.9") def vlmul: UInt = vlmul_mag
...
class CSRFile(perfEventSets: EventSets = new EventSets(), customCSRs: Seq[CustomCSR] = Nil)(implicit p: Parameters) extends CoreModule()(p) with HasCoreParameters {
  val io = new CSRFileIO {
    override val counters = Vec(28, new PerfCounterIO)
  ...
  val delegable_exceptions = UInt(Seq(Causes.misaligned_fetch, Causes.fetch_page_fault, Causes.breakpoint, Causes.load_page_fault, Causes.store_page_fault, Causes.misaligned_load, Causes.misaligned_store, Causes.illegal_instruction, Causes.user_ecall).map(1 << (_)).sum)
  ...
  val reg_mtvec = mtvecInit match {
    case Some(addr) =>
      Reg(init = UInt(addr, mtvecWidth))
    case None =>
      Reg(UInt(width = mtvecWidth))
  }
  ...
  val (nmi_interrupts, nmiFlag) = io.interrupts.nmi.map(nmi => ((nmi.unmi && reg_unmie) << CSR.unmiIntCause | (nmi.rnmi && reg_rnmie) << CSR.rnmiIntCause | io.interrupts.buserror.map(_ << CSR.rnmiBEUCause).getOrElse(0.U), !io.interrupts.debug && (nmi.unmi && reg_unmie || nmi.rnmi && reg_rnmie))).getOrElse(0.U, false.B)
  ...
  val isaMaskString = (if (usingMulDiv) "M" else "") + (if (usingAtomics) "A" else "") + (if (fLen >= 32) "F" else "") + (if (fLen >= 64) "D" else "") + (if (usingVector) "V" else "") + (if (usingBitManip) "B" else "") + (if (usingCompressed) "C" else "")
  val isaString = (if (coreParams.useRVE) "E" else "I") + isaMaskString + "X" + (if (usingSupervisor) "S" else "") + (if (usingUser) "U" else "")
  ...
  val debug_csrs = if (!usingDebug) LinkedHashMap() else LinkedHashMap[Int, Bits](CSRs.dcsr -> reg_dcsr.asUInt, CSRs.dpc -> readEPC(reg_dpc).sextTo(xLen), CSRs.dscratch -> reg_dscratch.asUInt) ++ reg_dscratch1.map(r => CSRs.dscratch1 -> r)
  val read_mnstatus = WireInit(0.U.asTypeOf(new MStatus()))
  read_mnstatus.mpp := io.status.mpp
  val nmi_csrs = if (!usingNMI) LinkedHashMap() else LinkedHashMap[Int, Bits](CSRs.mnscratch -> reg_mnscratch, CSRs.mnepc -> readEPC(reg_mnepc).sextTo(xLen), CSRs.mncause -> reg_mncause, CSRs.mnstatus -> read_mnstatus.asUInt)
  val context_csrs = LinkedHashMap[Int, Bits]() ++ reg_mcontext.map(r => CSRs.mcontext -> r) ++ reg_scontext.map(r => CSRs.scontext -> r)
  val read_fcsr = Cat(reg_frm, reg_fflags)
  val fp_csrs = LinkedHashMap[Int, Bits]() ++ usingFPU.option(CSRs.fflags -> reg_fflags) ++ usingFPU.option(CSRs.frm -> reg_frm) ++ (usingFPU || usingVector).option(CSRs.fcsr -> read_fcsr)
  val read_vcsr = Cat(reg_vxrm.getOrElse(0.U), reg_vxsat.getOrElse(0.U))
  val vector_csrs = if (!usingVector) LinkedHashMap() else LinkedHashMap[Int, Bits](CSRs.vxsat -> reg_vxsat.get, CSRs.vxrm -> reg_vxrm.get, CSRs.vcsr -> read_vcsr, CSRs.vstart -> reg_vstart.get, CSRs.vtype -> reg_vconfig.get.vtype.asUInt, CSRs.vl -> reg_vconfig.get.vl, CSRs.vlenb -> (vLen / 8).U)
  if (usingSupervisor) {
  ...
  val performanceCounters = new PerformanceCounters(perfEventSets, this, 28, true)
  buildMappings()
  ...
  val decoded_addr = read_mapping map ({
    case (k, v) =>
      k -> (io.rw.addr === k)
  })
  val wdata = readModifyWriteCSR(io.rw.cmd, io.rw.rdata, io.rw.wdata)
  val system_insn = io.rw.cmd === CSR.I
  val decode_table = Seq(SCALL -> List(Y, N, N, N, N, N), SBREAK -> List(N, Y, N, N, N, N), MRET -> List(N, N, Y, N, N, N), CEASE -> List(N, N, N, Y, N, N), WFI -> List(N, N, N, N, Y, N)) ++ usingDebug.option(DRET -> List(N, N, Y, N, N, N)) ++ usingNMI.option(MNRET -> List(N, N, Y, N, N, N)) ++ coreParams.haveCFlush.option(CFLUSH_D_L1 -> List(N, N, N, N, N, N)) ++ usingSupervisor.option(SRET -> List(N, N, Y, N, N, N)) ++ usingVM.option(SFENCE_VMA -> List(N, N, N, N, N, Y))
  val insn_call :: insn_break :: insn_ret :: insn_cease :: insn_wfi :: insn_sfence :: Nil = DecodeLogic(io.rw.addr << 20, decode_table(0)._2.map(x => X), decode_table).map(dl => system_insn && dl.asBool)
  for (io_dec <- io.decode) {
    def decodeAny(m: LinkedHashMap[Int, Bits]): Bool = m.map({
      case (k: Int, _: Bits) =>
        io_dec.csr === k
    }).reduce(_ || _)
    def decodeFast(s: Seq[Int]): Bool = DecodeLogic(io_dec.csr, s.map(_.U), (read_mapping -- s).keys.toList.map(_.U))
    val _ :: is_break :: is_ret :: _ :: is_wfi :: is_sfence :: Nil = DecodeLogic(io_dec.csr << 20, decode_table(0)._2.map(x => X), decode_table).map(_.asBool)
    ...
    when(reg_mstatus.prv < io_dec.csr(9, 8) || !decodeAny(read_mapping) || io_dec.csr === CSRs.satp && !allow_sfence_vma || decodeFast(debug_csrs.keys.toList) && !reg_debug || decodeFast(vector_csrs.keys.toList) && io_dec.vector_illegal || io_dec.fp_csr && io_dec.fp_illegal) {
    ...
    io_dec.system_illegal := reg_mstatus.prv < io_dec.csr(9, 8) || is_wfi && !allow_wfi || is_ret && !allow_sret || is_ret && io_dec.csr(10) && io_dec.csr(7) && !reg_debug || is_sfence && !allow_sfence_vma
  }
  val cause = Mux(insn_call, reg_mstatus.prv + Causes.user_ecall, Mux[UInt](insn_break, Causes.breakpoint, io.cause))
  ...
  val debugEntry = p(DebugModuleKey).map(_.debugEntry).getOrElse(BigInt(2048))
  val debugException = p(DebugModuleKey).map(_.debugException).getOrElse(BigInt(2056))
  ...
  if (xLen == 32) io.status.sd_rv32 := io.status.sd
  ...
  io.interrupts.nmi.map(nmi => when(nmi.unmi || nmi.rnmi) {
    reg_wfi := false
  })
  when(io.retire(0) || exception) {
    reg_singleStepped := true
  }
  when(!io.singleStep) {
  ...
  for (i <- 0 until xLen) {
    val supported_exceptions: BigInt = 2302 | (if (usingCompressed && !coreParams.misaWritable) 0 else 1) | (if (usingUser) 256 else 0) | (if (usingSupervisor) 512 else 0) | (if (usingVM) 45056 else 0)
  ...
  for (bp <- reg_bp drop nBreakpoints) bp := new BP().fromBits(0)
  for (pmp <- reg_pmp) {
    pmp.cfg.res := 0
    when(reset) {
      pmp.reset()
  ...
  def buildMappings() = {
    read_mapping += CSRs.tselect -> reg_tselect
    read_mapping += CSRs.tdata1 -> reg_bp(reg_tselect).control.asUInt
    read_mapping += CSRs.tdata2 -> reg_bp(reg_tselect).address.sextTo(xLen)
    read_mapping += CSRs.tdata3 -> reg_bp(reg_tselect).textra.asUInt
    read_mapping += CSRs.misa -> reg_misa
    read_mapping += CSRs.mstatus -> read_mstatus
    read_mapping += CSRs.mtvec -> read_mtvec
    read_mapping += CSRs.mip -> read_mip
    read_mapping += CSRs.mie -> reg_mie
    read_mapping += CSRs.mscratch -> reg_mscratch
    read_mapping += CSRs.mepc -> readEPC(reg_mepc).sextTo(xLen)
    read_mapping += CSRs.mtval -> reg_mtval.sextTo(xLen)
    read_mapping += CSRs.mcause -> reg_mcause
    read_mapping += CSRs.mhartid -> io.hartid
    ...
    if (reg_pmp.nonEmpty) {
      ...
      for (i <- 0 until read_pmp.size by pmpCfgPerCSR) read_mapping += CSRs.pmpcfg0 + pmpCfgIndex(i) -> read_pmp.map(_.cfg).slice(i, i + pmpCfgPerCSR).asUInt
      for ((pmp, i) <- read_pmp.zipWithIndex) read_mapping += CSRs.pmpaddr0 + i -> pmp.readAddr
    }
    (customCSRs zip reg_custom).map({
      case (csr, reg) =>
    ...
    when(csr_wen) {
      ...
      when(decoded_addr(CSRs.misa)) {
        ...
        when(!usingCompressed || !io.pc(1) || wdata('c' - 'a')) {
          if (coreParams.misaWritable) reg_misa := ~(~wdata | !f << 'd' - 'a') & mask | reg_misa & ~mask
        }
      }
      ...
      if (mtvecWritable) when(decoded_addr(CSRs.mtvec)) {
        reg_mtvec := wdata
      }
      ...
      if (usingDebug) {
        ...
        reg_dscratch1.foreach {
          r => when(decoded_addr(CSRs.dscratch1)) {
            r := wdata
          }
        }
      }
      if (usingSupervisor) {
        ...
        when(decoded_addr(CSRs.scause)) {
          reg_scause := wdata & UInt((BigInt(1) << xLen - 1) + 31)
        }
      ...
      reg_mcontext.foreach {
        r => when(decoded_addr(CSRs.mcontext)) {
          r := wdata
        }
      }
      reg_scontext.foreach {
        r => when(decoded_addr(CSRs.scontext)) {
          r := wdata
        }
      }
      if (reg_pmp.nonEmpty) for (((pmp, next), i) <- (reg_pmp zip reg_pmp.tail :+ reg_pmp.last).zipWithIndex) {
        ...
        when(decoded_addr(CSRs.pmpcfg0 + pmpCfgIndex(i)) && !pmp.cfgLocked) {
          ...
          if (pmpGranularity.log2 > PMP.lgAlign) pmp.cfg.a := Cat(newCfg.a(1), newCfg.a.orR)
        }
  ...
  def legalizePrivilege(priv: UInt): UInt = if (usingSupervisor) Mux(priv === PRV.H, PRV.U, priv) else if (usingUser) Fill(2, priv(0)) else PRV.M
  def trimPrivilege(priv: UInt): UInt = if (usingSupervisor) priv else legalizePrivilege(priv)
